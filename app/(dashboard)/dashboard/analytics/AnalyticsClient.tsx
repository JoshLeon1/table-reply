  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      const addFooter = (pageNum: number, total: number) => {
        ...
        doc.text(`Page ${pageNum} of ${total}`, 190, 290, { align: 'right' })
      }
      ...
      // Accent bar
      doc.setFillColor(224, 90, 40)
      doc.rect(0, 0, 6, 297, 'F')
      doc.setTextColor(224, 90, 40)
      doc.setFontSize(10)
      doc.setFont('helvetica', 'bold')
      doc.text('TABLEREPLY', 22, 30)
      doc.setTextColor(255, 255, 255)
      doc.setFontSize(28)
      doc.text(restaurantName, 22, 55, { maxWidth: 166 })
      doc.setFontSize(14)
      doc.setTextColor(160, 160, 160)
      doc.text(`${month} · Review Analytics Report`, 22, 75)
      // Divider
      doc.setDrawColor(40, 40, 40)
      doc.setLineWidth(0.5)
      doc.line(22, 83, 190, 83)
      // Key numbers
      const highlights = [
        { label: 'Total Reviews', val: String(totalReviews) },
        { label: 'Avg Rating', val: `${avgRating.toFixed(1)} ★` },
        { label: 'Response Rate', val: `${responseRate}%` },
        { label: 'Replies Sent', val: String(approvedCount) },
      ]
      highlights.forEach(({ label, val }, i) => {
        const x = 22 + (i % 2) * 90
        const y = 100 + Math.floor(i / 2) * 30
        doc.setFontSize(22)
        doc.setFont('helvetica', 'bold')
        doc.setTextColor(255, 255, 255)
        doc.text(val, x, y)
      ...
      if (criticalUnanswered > 0) {
        ...
        doc.setFont('helvetica', 'bold')
        doc.setTextColor(224, 90, 40)
        doc.text(`⚠  ${criticalUnanswered} critical review${criticalUnanswered !== 1 ? 's' : ''} need a reply`, 22, 175)
      }
      ...
      doc.setFontSize(18)
      doc.setFont('helvetica', 'bold')
      doc.text('Executive Summary', 22, 30)

      let y = 48
      doc.setFontSize(12)
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(10, 10, 10)
      doc.text('Rating Distribution', 22, y); y += 8
      doc.setFontSize(10)
      doc.setFont('helvetica', 'normal')
      ...
      ;[5, 4, 3, 2, 1].forEach((star) => {
        ...
        doc.text(`${star}★  ${count} reviews  (${pct}%)`, 22, y)
        // Bar
        doc.setFillColor(228, 222, 216)
        doc.rect(80, y - 3.5, 90, 4, 'F')
        doc.setFillColor(224, 90, 40)
        doc.rect(80, y - 3.5, Math.round((pct / 100) * 90), 4, 'F')
        y += 9
      })
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(10, 10, 10)
      doc.text('Month-by-Month Volume', 22, y); y += 8
      doc.setFontSize(10)
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(87, 83, 78)
      trendData.filter((d) => d.count > 0).forEach((d) => {
        doc.text(`${d.month.padEnd(6)}  ${d.count} review${d.count !== 1 ? 's' : ''}  · avg ${d.rating?.toFixed(1) ?? '–'} ★`, 22, y)
        y += 7
      })
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(10, 10, 10)
      doc.text('Response Rate by Rating', 22, y); y += 8
      doc.setFontSize(10)
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(87, 83, 78)
      ;[1, 2, 3, 4, 5].forEach((star) => {
        ...
        doc.text(`${star}★  ${d.replied}/${d.total} replied (${pct}%)`, 22, y)
        y += 7
      })
      ...
      if (themes.praised.length || themes.complaints.length || themes.opportunities.length) {
        ...
        doc.setFontSize(18)
        doc.setFont('helvetica', 'bold')
        doc.text('AI-Powered Insights', 22, 30)

        y = 48
        const sections = [
          { title: 'Customers love', items: themes.praised, color: [34, 197, 94] as [number,number,number] },
          { title: 'Frequently mentioned', items: themes.complaints, color: [160, 160, 160] as [number,number,number] },
          { title: 'Top opportunities', items: themes.opportunities, color: [224, 90, 40] as [number,number,number] },
        ]
        for (const section of sections) {
          ...
          doc.text(section.title, 22, y); y += 8
          doc.setFontSize(10)
          doc.setFont('helvetica', 'normal')
          doc.setTextColor(87, 83, 78)
      ...
      if (hasDowData || topKeywords.length) {
        ...
        doc.setFontSize(18)
        doc.setFont('helvetica', 'bold')
        doc.text('Activity Patterns', 22, 30)
        ...
        if (hasDowData) {
          ...
          doc.setFont('helvetica', 'bold')
          doc.setTextColor(10, 10, 10)
          doc.text('Reviews by Day of Week', 22, y); y += 8
          doc.setFontSize(10)
          doc.setFont('helvetica', 'normal')
          doc.setTextColor(87, 83, 78)
          ...
          dowData.forEach((d) => {
            if (!d.count) return
            doc.text(`${d.label.padEnd(4)}  ${d.count} review${d.count !== 1 ? 's' : ''}  · avg ${d.avgRating}★`, 22, y)
            doc.setFillColor(228, 222, 216)
            doc.rect(90, y - 3.5, 80, 4, 'F')
            doc.setFillColor(224, 90, 40)
            doc.rect(90, y - 3.5, Math.round((d.count / maxC) * 80), 4, 'F')
            y += 8
          })
          y += 6
        }
        ...
        if (topKeywords.length) {
          ...
          doc.setFont('helvetica', 'bold')
          doc.setTextColor(10, 10, 10)
          doc.text('Top Keywords in Reviews', 22, y); y += 8
          doc.setFontSize(10)
          doc.setFont('helvetica', 'normal')
          doc.setTextColor(87, 83, 78)
          topKeywords.slice(0, 15).forEach((kw, i) => {
            const col = i < 8 ? 22 : 110
            const row = i < 8 ? y + i * 7 : y + (i - 8) * 7
            doc.text(`${kw.word} (${kw.count})`, col, row)
          })
        }
      }

      // ── Page 5: Best & Worst Reviews ─────────────────────────────────────
      if (bestReview || worstReview) {
        ...
        doc.setFontSize(18)
        doc.setFont('helvetica', 'bold')
        doc.text('Notable Reviews', 22, 30)
        ...
        if (bestReview) {
          ...
          doc.setFont('helvetica', 'bold')
          doc.setTextColor(10, 10, 10)
          doc.text(`Best review — ${bestReview.star_rating}★ by ${bestReview.reviewer_name}`, 22, y); y += 8
          doc.setFontSize(10)
          doc.setFont('helvetica', 'normal')
          doc.setTextColor(87, 83, 78)
          const bestLines = doc.splitTextToSize(`"${bestReview.review_text}"`, 166)
          doc.text(bestLines, 22, y)
          y += bestLines.length * 6 + 12
        }
        if (worstReview && worstReview.id !== bestReview?.id) {
          doc.setFontSize(12)
          doc.setFont('helvetica', 'bold')
          doc.setTextColor(10, 10, 10)
          doc.text(`Lowest-rated — ${worstReview.star_rating}★ by ${worstReview.reviewer_name}`, 22, y); y += 8
          doc.setFontSize(10)
          doc.setFont('helvetica', 'normal')
          doc.setTextColor(87, 83, 78)
          const worstLines = doc.splitTextToSize(`"${worstReview.review_text}"`, 166)
          doc.text(worstLines, 22, y)
        }
      }
      ...
      const filename = `${restaurantName.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-${month.replace(' ', '-').toLowerCase()}-report.pdf`
      doc.save(filename)
    } catch (err) {
  ...
  return (
    <div className="space-y-12 pb-20" onClick={() => showDownloadMenu && setShowDownloadMenu(false)}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold text-[#111] leading-tight">{restaurantName}</h1>
        ...
        <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
          ...
          <div className="relative">
            ...
            >
              ...
              {downloadingReport ? 'Generating PDF…' : downloadingCsv ? 'Exporting CSV…' : 'Export'}
      ...
      <div className="rounded-3xl bg-[#0A0A0A] overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
          ...
          <div className="order-1 md:order-2 flex flex-col items-center justify-center px-6 sm:px-8 py-8 sm:py-12 text-center border-b md:border-b-0 border-white/[0.07]">
            <p className="text-white leading-none mb-4" style={{ fontSize: 'clamp(64px, 10vw, 120px)', fontWeight: 700, letterSpacing: '-0.02em' }}>
              {avgRating.toFixed(1)}
            </p>
              ...
              {[5, 4, 3, 2, 1].map((star) => {
                ...
                return (
                  <div key={star} className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] text-white/40 w-14 flex-shrink-0 text-right">{count} · {Math.round(pct)}%</span>
                  </div>
                )
              })}
            </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            sub: criticalUnanswered === 0 ? 'All low ratings addressed' : '1–2★ with no reply',
            color: criticalUnanswered === 0 ? 'text-emerald-600' : 'text-red-500',
          },
        ].map((stat, i) => (
          <div key={i} className={`bg-white rounded-2xl border border-[#E4DED8] p-4 shadow-card stagger-${i + 1} animate-fade-up`}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.10em] text-[#A8A29E] mb-2">{stat.label}</p>
            <p className={`text-xl sm:text-2xl font-bold leading-none mb-1 ${stat.color}`}>{stat.value}</p>
            <p className="text-[11px] text-[#A8A29E]">{stat.sub}</p>
          </div>
        ))}
      </div>
          ...
          <div className="space-y-3">
            {([
              ...
              <div key={label} className="flex items-center gap-3">
                <span className={`text-[12px] flex-shrink-0 w-28 truncate ${highlight ? 'font-semibold text-[#111]' : 'text-[#AAA]'}`}>{label}</span>
            ...
            {[1, 2, 3, 4, 5].reverse().map((star) => {
              ...
              return (
                <div key={star} className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] text-[#A8A29E] w-20 flex-shrink-0">{d.replied}/{d.total} replied</span>
                </div>
              )
            })}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E4DED8] p-5 sm:p-6 shadow-card">
            ...
            <div className="flex flex-wrap gap-2">
              {topKeywords.map(({ word, count }, i) => {
                ...
                const size = 0.6 + (count / maxCount) * 0.6
