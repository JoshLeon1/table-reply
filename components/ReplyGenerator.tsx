...
function PersonalizationBadge({ score }: { score: ReturnType<typeof getPersonalizationScore> }) {
  ...
  if (score === 'personalized') {
    return <span className="text-[11px] font-medium text-[#57534E] bg-[#F8F6F3] border border-[#E4DED8] px-2 py-0.5 rounded-full">Personalized</span>
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-[#E4DED8] shadow-card p-5 sm:p-6">
        <div className="space-y-5">
          ...
          <div>
            <label className="block text-[13px] font-semibold text-[#111111] mb-2">Review</label>
            <textarea
              value={reviewText}
              onChange={(e) => setReviewText(e.target.value)}
              placeholder="Paste the customer review here…"
              rows={5}
              className="w-full px-3.5 py-3 rounded-xl border border-[#E4DED8] text-[#111111] text-[14px] placeholder:text-[#C4BEB8] bg-[#F8F6F3] hover:bg-white focus:bg-white resize-y focus:outline-none focus:ring-2 focus:ring-[#E05A28]/10 focus:border-[#E05A28]/50 transition-all duration-150 leading-relaxed min-h-[100px]"
            />
          </div>

          <div className="flex flex-col gap-4">
            <div>
              <label className="block text-[13px] font-semibold text-[#111111] mb-2">Rating</label>
              <StarRating value={starRating} onChange={setStarRating} />
            </div>

            <div>
              <label className="block text-[13px] font-semibold text-[#111111] mb-2">Platform</label>
                ...
                {platforms.map((p) => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-[13px] font-medium transition-all duration-150 min-h-[38px] ${
                      platform === p
                        ? 'bg-[#E05A28]/10 border border-[#E05A28]/30 text-[#E05A28] font-semibold'
                        : 'bg-[#F8F6F3] text-[#57534E] hover:bg-[#F3F0EC] border border-[#E4DED8]'
                    }`}
                  >
                    {p}
                  </button>
          ...
          <kbd className="hidden sm:inline-flex items-center gap-0.5 bg-[#F8F6F3] border border-[#E4DED8] text-[#A8A29E] text-[10px] font-mono px-1.5 py-0.5 rounded-md select-none">
            ⌘ Enter
          </kbd>
          </div>
        </div>
      ...
      {generatedReply && (
        <div className="bg-white rounded-2xl border border-[#E4DED8] border-l-[3px] border-l-[#E05A28] shadow-card overflow-hidden animate-fade-up">
          <div className="px-4 sm:px-6 py-3.5 border-b border-[#EDE9E4]">
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[13px] font-semibold text-[#111111]">Generated reply</span>
              </div>
          ...
          <div className="px-5 sm:px-6 py-5">
            <p className="text-[#111111] text-[14px] leading-relaxed whitespace-pre-wrap">{generatedReply}</p>
          </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-all active:scale-[0.97] disabled:opacity-40 ${
                    activeTone === value
                      ...
                      : 'bg-[#F8F6F3] text-[#57534E] border border-[#E4DED8] hover:bg-[#F3F0EC]'
                  }`}
                >
                  {label}
                </button>
          ...
          <div className="px-5 sm:px-6 py-3.5 border-t border-[#EDE9E4] flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={`p-1.5 rounded-lg transition-all ${feedback === 'good' ? 'bg-emerald-50 text-emerald-600' : 'text-[#C4BEB8] hover:text-emerald-600 hover:bg-emerald-50'}`}
              >
              ...
              <button
                ...
                className={`p-1.5 rounded-lg transition-all ${feedback === 'bad' ? 'bg-red-50 text-red-500' : 'text-[#C4BEB8] hover:text-red-500 hover:bg-red-50'}`}
              >
          ...
          <div className="px-5 sm:px-6 py-4 border-t border-[#EDE9E4]">
            <button
              ...
              className="flex items-center gap-2 text-[13px] font-medium text-[#A8A29E] hover:text-[#111111] disabled:opacity-40 transition-colors"
            >
              {socialLoading ? (
            ...
            {socialPost && (
              <div className="mt-4 p-4 bg-[#F8F6F3] rounded-xl border border-[#E4DED8]">
                ...
                <p className="text-[13px] text-[#57534E] whitespace-pre-wrap leading-relaxed">{socialPost}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {loading && !generatedReply && (
        <div className="bg-white rounded-2xl border border-[#E4DED8] shadow-card p-5 sm:p-6 animate-fade-in">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div className="animate-pulse bg-[#F3F0EC] rounded h-3.5 w-32" />
          </div>
          <div className="space-y-2.5">
            <div className="animate-pulse bg-[#F3F0EC] rounded h-3.5 w-full" />
            <div className="animate-pulse bg-[#F3F0EC] rounded h-3.5 w-[92%]" />
            <div className="animate-pulse bg-[#F3F0EC] rounded h-3.5 w-[85%]" />
            <div className="animate-pulse bg-[#F3F0EC] rounded h-3.5 w-[78%]" />
            <div className="animate-pulse bg-[#F3F0EC] rounded h-3.5 w-[60%]" />
          </div>
        </div>
      )}
          ...
          <div className="border-t border-[#EDE9E4] divide-y divide-[#EDE9E4]">
            {EXAMPLES.map((ex, i) => (
              <div key={i} className="px-5 sm:px-6 py-5">
                ...
                <p className="text-[12px] text-[#A8A29E] italic mb-3 leading-relaxed">"{ex.review}"</p>
                <div className="bg-[#F8F6F3] rounded-xl p-4 border border-[#E4DED8]">
                  ...
                  <p className="text-[12px] text-[#57534E] leading-relaxed whitespace-pre-wrap">{ex.reply}</p>
                </div>
              </div>
            ))}
          </div>
